...
public class VisitorBuildSymbolTableM2SP extends VisitorBuildSymbolTable{
    /**
     * f0 -> "class"
     * f1 -> Identifier()
     * f2 -> "{"
    ...
    public void visit(MainClass n, EntryInfo argu) // here argu should be the topSymbolTable
    {
    	...
    	EntryInfoClassM2SP mainClassInfo = new EntryInfoClassM2SP();
    	mainClassInfo.set_id_info(n.f1);
    	
    	
    	//then we need to manually build all things for the main method...
    	EntryInfoMethodM2SP mainMethodInfo = new EntryInfoMethodM2SP();
    ...
    {
    	EntryInfoClassM2SP classInfo = new EntryInfoClassM2SP();
    	classInfo.set_id_info(n.f1);
    	
    	argu.c_put_m2sp(classInfo.get_name(), classInfo);
    ...
    {
    	EntryInfoMethodM2SP methodInfo = new EntryInfoMethodM2SP();
    	methodInfo.set_id_info(n.f2);
    	methodInfo.set_rtn_type(n.f1);
    	methodInfo.set_belong_class_name(argu.get_name());
    	
    	argu.m_put_m2sp(methodInfo.get_name(), methodInfo);
    	((EntryInfoClassM2SP) argu).m_zone_put(methodInfo.get_name(), methodInfo.get_belong_class_name() + "_" + methodInfo.get_name());
    ...
    public void visit(VarDeclaration n, EntryInfo argu)//here the argu should be an EntryInfoClass or an EntryInfoMethod
    {
    	...
    	{
    		((EntryInfoMethodM2SP) argu).add_local_defined_variable(variableInfo.get_name());
    	}
